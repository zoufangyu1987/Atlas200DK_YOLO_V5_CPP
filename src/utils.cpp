...
std::vector<BBox> Utils::PostProcess(const string &path, aclmdlDataset *modelOutput, aclmdlDesc* modelDesc)
{
    ...
    std::cout<<"==a=="<<path<<"==b=="<<modelDesc<<"==c=="<<modelOutput<<std::endl;
...
//Result ObjectDetect::Postprocess(ImageData& image, aclmdlDataset* modelOutput,const string& origImagePath) 
std::vector<BBox> Utils::PostProcess(const string &path, aclmdlDataset *modelOutput, aclmdlDesc* modelDesc, shared_ptr<ImageDesc>& imageData)
{
    ...
    //H = Image.rows;
    //W = Image.cols;
    H = imageData->img_height;
    W = imageData->img_width;
    float widthScale = float(W) / float(MODEL_INPUT_WIDTH);
    float heightScale = float(H) / float(MODEL_INPUT_HEIGHT);

    //std::cout<<"aaaa"<<widthScale<< " "<<heightScale<<std::endl;
    //std::cout<<"bbbb"<<MODEL_INPUT_WIDTH<< " "<<MODEL_INPUT_HEIGHT<<std::endl;
    ...
    for (uint32_t i = 0; i < totalBox; i++) {
        ...
        float score = (detectData[i*85 + SCORE]);
        if (score < 0.3) 
	   continue;
        //std::cout<<score<<std::endl;

        int center_x = detectData[i*85 + TOPLEFTX] * widthScale;
        int center_y = detectData[i*85 + TOPLEFTY] * heightScale;
        int width = detectData[i*85 + BOTTOMRIGHTX] * widthScale;
        int height = detectData[i*85 +  BOTTOMRIGHTY] * heightScale;

        boundBox.rect.ltX = center_x - width/2;
        boundBox.rect.ltY = center_y - height/2;
        boundBox.rect.rbX = center_x + width/2;
        boundBox.rect.rbY = center_y + height/2;
	boundBox.cls = 11;//FIXME

        //std::cout<<"bbb"<<boundBox.rect.ltX<<boundBox.rect.ltY<<boundBox.rect.rbX<<boundBox.rect.rbY<<std::endl;

        //uint32_t objIndex = (uint32_t)detectData[i * LABEL];
        //boundBox.text = Label[objIndex];
        //printf("%d %d %d %d %f %s\n", boundBox.rect.ltX, boundBox.rect.ltY, boundBox.rect.rbX, boundBox.rect.rbY, score, boundBox.text.c_str());
	//printf("%d %d %d %d %f\n", boundBox.rect.ltX, boundBox.rect.ltY, boundBox.rect.rbX, boundBox.rect.rbY, score);
...
bool Utils::PreProcess(shared_ptr<ImageDesc>& imageData, const string& imageFile) {
    ...
    */
    ...
    imageData->img_width = image.cols;
    imageData->img_height = image.rows;                        //===========================//
